  ...
  switch (type) {
    case 0:
      ...
      setup_json.decks.forEach(element => {
        $("#modalForm > .modal-dialog > .modal-content > .modal-body > select").html(`<option>${element.deck_name}</option>`);
      });
      $("#modalForm > .modal-dialog > .modal-content > .modal-footer > .btn-primary").html('Confirm');
      break;
    // case 1:
    //   modal_state = 1;
    //   $("#modalForm > .modal-dialog > .modal-content > .modal-header > .modal-title").html('<p>Create a new deck</p>');
